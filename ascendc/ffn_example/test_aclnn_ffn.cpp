...
int main() {
    ...
    // 定义输入输出形状
    // 假设输入x的形状为[batch_size, seq_len, hidden_size]
    std::vector<int64_t> xShape = {2, 16, 128}; // [batch=2, seq=16, hidden=128]
    // weight1的形状为[hidden_size, intermediate_size]
    std::vector<int64_t> weight1Shape = {128, 512}; // [hidden=128, intermediate=512]
    // weight2的形状为[intermediate_size, hidden_size]
    std::vector<int64_t> weight2Shape = {512, 128}; // [intermediate=512, hidden=128]
    // bias1的形状为[intermediate_size]
    std::vector<int64_t> bias1Shape = {512}; // [intermediate=512]
    // bias2的形状为[hidden_size]
    std::vector<int64_t> bias2Shape = {128}; // [hidden=128]
    // 输出y的形状与输入x相同
    ...
    // 生成权重weight1数据
    size_t weight1Size = GetShapeSize(weight1Shape);
    std::vector<uint16_t> weight1HostData(weight1Size);
    for (size_t i = 0; i < weight1Size; ++i) {
        weight1HostData[i] = *reinterpret_cast<uint16_t*>(&dist(gen));
    }

    // 生成权重weight2数据
    size_t weight2Size = GetShapeSize(weight2Shape);
    std::vector<uint16_t> weight2HostData(weight2Size);
    for (size_t i = 0; i < weight2Size; ++i) {
        weight2HostData[i] = *reinterpret_cast<uint16_t*>(&dist(gen));
    }

    // 生成偏置bias1数据
    size_t bias1Size = GetShapeSize(bias1Shape);
    std::vector<uint16_t> bias1HostData(bias1Size);
    for (size_t i = 0; i < bias1Size; ++i) {
        bias1HostData[i] = *reinterpret_cast<uint16_t*>(&dist(gen));
    }

    // 生成偏置bias2数据
    size_t bias2Size = GetShapeSize(bias2Shape);
    std::vector<uint16_t> bias2HostData(bias2Size);
    for (size_t i = 0; i < bias2Size; ++i) {
        bias2HostData[i] = *reinterpret_cast<uint16_t*>(&dist(gen));
    }
    ...
    aclTensor *weight1Tensor = nullptr;
    void *weight1DeviceAddr = nullptr;
    ret = CreateAclTensor(weight1HostData, weight1Shape, &weight1Tensor, &weight1DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "CreateAclTensor for weight1 failed");

    aclTensor *weight2Tensor = nullptr;
    void *weight2DeviceAddr = nullptr;
    ret = CreateAclTensor(weight2HostData, weight2Shape, &weight2Tensor, &weight2DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "CreateAclTensor for weight2 failed");

    aclTensor *bias1Tensor = nullptr;
    void *bias1DeviceAddr = nullptr;
    ret = CreateAclTensor(bias1HostData, bias1Shape, &bias1Tensor, &bias1DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "CreateAclTensor for bias1 failed");

    aclTensor *bias2Tensor = nullptr;
    void *bias2DeviceAddr = nullptr;
    ret = CreateAclTensor(bias2HostData, bias2Shape, &bias2Tensor, &bias2DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "CreateAclTensor for bias2 failed");
    ...
    ret = aclnnFfnGetWorkspaceSize(xTensor, weight1Tensor, weight2Tensor, bias1Tensor, bias2Tensor,
                                 yTensor, 0, 0, &workspaceSize, nullptr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "aclnnFfnGetWorkspaceSize failed");
    ...
    // 执行FFN算子
    ret = aclnnFfn(workspaceAddr, workspaceSize, executor, stream, xTensor, weight1Tensor,
                  weight2Tensor, bias1Tensor, bias2Tensor, yTensor, 0, 0);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "aclnnFfn failed");
    ...
    ret = DestroyAclTensor<uint16_t>(weight1Tensor, weight1DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "DestroyAclTensor for weight1 failed");

    ret = DestroyAclTensor<uint16_t>(weight2Tensor, weight2DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "DestroyAclTensor for weight2 failed");

    ret = DestroyAclTensor<uint16_t>(bias1Tensor, bias1DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "DestroyAclTensor for bias1 failed");

    ret = DestroyAclTensor<uint16_t>(bias2Tensor, bias2DeviceAddr);
    CHECK_RET(ret == ACL_ERROR_NONE, 1, "DestroyAclTensor for bias2 failed");
    ...
    INFO_LOG("FFN example completed successfully");
    return 0;
}